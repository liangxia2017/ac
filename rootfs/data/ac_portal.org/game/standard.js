...
//  }
 ...
 function imgpreload(ims) {
  ...
  function menuswitch(obj) {
   ...
   tmp=new Array('game','highscore'); 
   tmp2=new Array();
   tmp2['game']=0;
   tmp2['highscore']='-160px';
   //tmp2['about']='-182px';
   for(i in tmp) {
  ...
  function ialert(iquestion,idoit,highscore) {
   document.getElementById('alertbox_content').innerHTML=iquestion+'<br style="clear:both" /><br /><input type="button" style="padding: 3px; font-size: 150%; font-weight: bold;" value="重玩" onclick="idismiss();eval(\''+idoit+'\'); return false;" ontouchstart="idismiss();eval(\''+idoit+'\'); return false;" />&nbsp;&nbsp;&nbsp;<input type="button" style="padding: 3px; font-size: 150%; font-weight: bold;" value="提交" onclick="game_submit('+highscore+')" />';
