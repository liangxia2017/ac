	...
	struct lease *lease;
{
	...
	if(count&&(lease -> binding_state==1))
	{
		//sprintf(cmd,"/usr/sbin/iptables -t nat -D PREROUTING -s %s -j ACCEPT > /dev/null 2>&1",piaddr (lease -> ip_addr));
		sprintf(cmd,"/ac/script/ipset_do 0 %s",piaddr (lease -> ip_addr));
		//system(cmd);
	}
	if (lease->starts &&
